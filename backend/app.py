...
if "Unnamed: 133" in df.columns: df = df.drop(columns=["Unnamed: 133"])
...
def find_file(filename):
    # Try frontend/patient/ then frontend/
...
@app.route("/symptoms")
def get_symptoms(): return jsonify(symptoms)
...
def predict():
    ...
    for s in sel:
        if s in symptoms: input_data[symptoms.index(s)]=1
    input_df=pd.DataFrame([input_data], columns=symptoms)
    pred=model.predict(input_df)
    disease=str(pred[0]).strip()
    try: doctor=get_recommended_doctor(disease)
    except: doctor="General Physician"
    try:
    ...
    try: add_prediction({"patientName":data.get("patientName"),"patientAge":data.get("patientAge"),"patientGender":data.get("patientGender"),"disease":disease,"confidence":conf,"severity":info.get("severity"),"doctor":doctor})
    except: pass
...
def chatbot():
    data=request.get_json()
    try: reply=get_reply(data.get("message",""))
    except: reply="Sorry"
...
def reg_api():
    ...
    if any(u["email"]==email for u in users): return jsonify({"success":False,"message":"Email exists"}),409
